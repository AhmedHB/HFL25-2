...
int readNumber(String prompt) {
  while (true) {
    ...
    if (input != null && validateNumericInput(input) == 'Valid') {
...
int readNumberWithRange(String prompt, int min, int max) {
  while (true) {
    stdout.write('$prompt: ');
    String? input = stdin.readLineSync();
    int? number = int.tryParse(input ?? '');
...
String readString(String prompt, List<String> validTypes, bool validation) {
  while (true) {
    ...
    if (!validation) {
      ...
      print(
        'Fel: validerings fel. Ange något av dem korrekta alternativen: $types',
      );
    }
  }
}
